...
async def run(payload: dict) -> None:
    ...
    if failure_type in DETERMINISTIC_CLASSIFICATIONS:
        ...
        llm_model = "gemini-2.0-flash" if provider_name == "gemini" else "claude-sonnet-4-5"
        logger.info(
            "diagnose_runtime_failure: Tier 2 — %s classified as %s via LLM",
            failure_type, classification,
        )

    # ── Phase 3: write classification result ─────────────────────────────────
    async with AsyncSessionLocal() as session:
        event = await session.get(RecoveryEvent, recovery_event_id)
        if event is None:
            logger.error(
    ...
    # ── Phase 4: enqueue recovery (outside session) ───────────────────────────
    async with AsyncSessionLocal() as session:
        await enqueue_job(
            session,
